...
class Backbone(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.output_channels = inp_channels
    ...
    def forward(self, x):
        ...
        x = torch.cat([x[0], \
                       F.interpolate(x[1], size=(x0_h, x0_w), mode='bilinear',align_corners=False), \
                       F.interpolate(x[2], size=(x0_h, x0_w), mode='bilinear',align_corners=False), \
                       F.interpolate(x[3], size=(x0_h, x0_w), mode='bilinear',align_corners=False)], 1)
        ...
        #         self.multi_level_layers_4x_heatmap[j](self.transition_cls(x))))
        #
        ...
        #         self.multi_level_layers_4x_regression[j](self.transition_reg(x))))
        #
    ...
    def init_weights(self, pretrained='', verbose=True):
        ...
        if os.path.isfile(pretrained):
            ...
            raise Warning("failed to load the pretrained model from {}".format(pretrained))


def get_backbone(cfg, is_train, **kwargs):
    backbone = Backbone(cfg, **kwargs)

    if is_train and cfg.MODEL.INIT_WEIGHTS:
        backbone.init_weights(cfg.MODEL.PRETRAINED, verbose=cfg.VERBOSE)
