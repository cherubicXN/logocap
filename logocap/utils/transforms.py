...
def resize_align_multi_scale(image, joints, mask, input_size, 
    ...
    if joints is not None:
        joints_resized[:, :, 0:2] = affine_joints(
                    joints[:, :, 0:2], trans,
                )
        result.append(joints_resized)
