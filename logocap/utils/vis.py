...
from lib.dataset import VIS_CONFIG
...
def add_joints(image, joints, color, dataset='COCO'):
    ...
    def link(a, b, color):
        if part_idx[a] < joints.shape[0] and part_idx[b] < joints.shape[0]:
            ...
            if jointa[2] > 0.05 and jointb[2] > 0.05:
                cv2.line(
                    image,
...
def save_valid_image_pred_gt(image, gt_joints, pred_joints, file_name, dataset='COCO'):
    if len(image.shape) == 4:
        image = image[0]
        image = image.mul(255)\
                     .clamp(0, 255)\
    ...
    joints_list = [gt_joints, pred_joints]
...
def make_valid_heatmaps(image, heatmaps):
