...
class HeatmapGeneratorNS():
    ...
    def __call__(self, joints, sgm, ct_sgm, height, width, bg_weight=1.0):
        assert self.num_joints == joints.shape[1], \
            'the number of joints should be %d' % self.num_joints

        hms = np.zeros((self.num_joints, height, width),
                       dtype=np.float32)
        ignored_hms = 2*np.ones((1, height, width),
                                dtype=np.float32)
...
class OffsetGeneratorNS():
    def __init__(self, num_joints, radius):
        self.num_joints_without_center = num_joints - 1
        self.num_joints = num_joints
        self.radius = radius

    def __call__(self, joints, area, output_h, output_w):
        assert joints.shape[1] == self.num_joints, \
            'the number of joints should be 18, 17 keypoints + 1 center joint.'

        offset_map = np.zeros((self.num_joints_without_center*2, output_h, output_w),
                              dtype=np.float32)
        weight_map = np.zeros((self.num_joints_without_center*2, output_h, output_w),
                              dtype=np.float32)
        area_map = np.zeros((output_h, output_w),
                            dtype=np.float32)
        ...
        for person_id, p in enumerate(joints):
            ...
            if ct_v < 1 or ct_x < 0 or ct_y < 0 \
                    or ct_x >= output_w or ct_y >= output_w:
                continue
