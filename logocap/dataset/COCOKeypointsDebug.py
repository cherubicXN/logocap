...
class CocoKeypoints(Dataset):
    ...
    def _get_anno_file_name(self):
        ...
        if 'test' in self.dataset:
            raise NotImplementedError('the debug dataset must contain the human annotations')
            return os.path.join(
                self.root,
                'annotations',
    ...
    def __getitem__(self, idx):
        ...
        target_t, ignored_t = self.heatmap_generator[0](joints_list[0],self.sigma[0][0],self.center_sigma,mask_list[0].shape[0],mask_list[0].shape[1],self.bg_weight[0][0])
        
        heatmaps_target = target_t.astype(np.float32)
        heatmaps_weight = mask_list[0]*ignored_t
        joints_with_centers = joints_list[0]
        offset_targets, offsets_weight = self.offset_generator[0](joints_with_centers,area,mask_list[0].shape[0],mask_list[0].shape[1])
