...
from logocap.utils.transforms import get_affine_transform
import copy
...
class Compose(object):
    ...
    def __call__(self, image, mask, joints, area,meta):
        for t in self.transforms:
            image, mask, joints, area, meta = t(image, mask, joints, area,meta)
        return image, mask, joints, area, meta
...
class ToTensor(object):
    def __call__(self, image, mask, joints, area,meta):
        return F.to_tensor(image), mask, joints, area,meta
...
class AffineTransformDebug(object):
    def __init__(self,
                 input_size,
                 output_size,
                 scale_type):
    ...
    def __call__(self, image, mask, joints, area,meta):
        ...
        min_input_size = int((self.input_size + 63)//64*64)
        if width < height:
            w_resized = int(min_input_size)
            h_resized = int((min_input_size/width*height+63)//64*64)
            scale_w = width/200.0
            scale_h = h_resized / w_resized * width / 200.0
        else:
            h_resized = int(min_input_size)
            w_resized = int((min_input_size/height*width+63)//64*64)
            scale_h = height/200.0
            scale_w = w_resized / h_resized * height /200.0
        ...
        for i, _output_size in enumerate(self.output_size):
            if width > height:
                _output_size_h = _output_size
                _output_size_w = int((_output_size/h_resized)*w_resized)
            else:
                _output_size_w = _output_size
                _output_size_h = int((_output_size/w_resized)*h_resized)
            mat_output, _ = self._get_affine_matrix(
                center, scale, (_output_size_w, _output_size_h))

            mat_output = mat_output[:2]
            mask[i] = cv2.warpAffine(
                (mask[i]*255).astype(np.uint8), mat_output,
                (_output_size_w, _output_size_h)
            ) / 255
            mask[i] = (mask[i] > 0.5).astype(np.float32)
        ...
        mat_input, final_scale = self._get_affine_matrix(
            center, scale, (w_resized, h_resized),
        )
        
        meta['transform'] = copy.deepcopy(mat_input)
        mat_input = mat_input[:2]
        area = area*final_scale
        image = cv2.warpAffine(
            image, mat_input, (w_resized, h_resized),
        )
